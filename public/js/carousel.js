	...
	return {
		...
		init : function(s){
			...
			window.addEventListener('resize',function(){
				var arr = that.targetElement.childNodes[0].childNodes;
				var width = that.targetElement.clientWidth + 'px';
		...
		show : function (){
			...
			for(var i=0;i<this.img.length;i++){
				this.bx.innerHTML += '<div style="width:' + (this.targetElement.offsetWidth) + 'px;height:100%;float:left;"><a href="'+this.img[i].target+'" ' + (this.openInNewTab?'target="_blank"':'') +'><img src="'+this.img[i].src+'" style="'+(this.stretch?'':'max-')+'width:100%;'+(this.stretch?'':'max-')+'height:100%;"></a></div>';
			}
